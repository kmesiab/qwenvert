...
class TestNetworkIsolation:
    ...
    def test_config_generator_only_creates_localhost_urls(self):
        ...
        ollama_model = Model(
            id="test", display_name="Test", family="test",
            size_b=7.0, quantization="Q4_K_M", backend=Backend.OLLAMA,
            backend_model_id="test:7b", context_length=4096,
            min_ram_gb=8, recommended_ram_gb=16,
        )

        config = ConfigGenerator(ollama_model, hardware).generate_qwenvert_config()

        assert "localhost" in config.backend_url or "127.0.0.1" in config.backend_url, \
            f"Backend URL {config.backend_url} is not localhost!"

        assert config.adapter_host in ["127.0.0.1", "localhost"], \
    ...
    def test_environment_variables_safe(self):
        ...
        assert api_key == "local-qwen", \
            "API key should be placeholder 'local-qwen'"
