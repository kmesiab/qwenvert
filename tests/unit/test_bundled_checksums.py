...
class TestBundledChecksums:
    ...
    def test_bundled_checksum_file_exists(self):
        ...
        ), f"Checksums directory should exist: {checksums_dir}"
        ...
        assert (
            "arm64" in content or "x64" in content
        ), "Checksum file should contain architecture identifiers"
    ...
    def test_get_bundled_checksum_arm64(self, binary_manager):
        ...
        checksum = binary_manager._get_bundled_checksum(
            version="b8054", filename="llama-b8054-bin-macos-arm64.tar.gz"
        )
        ...
        assert (
            checksum
            == "b2d02aff34fdcbadacc6f2f7f5d043404769709aedf3bcbc441ef3f315e73565"
        )

    def test_get_bundled_checksum_x64(self, binary_manager):
        """Test retrieving x64 checksum from bundled file."""
        checksum = binary_manager._get_bundled_checksum(
            version="b8054", filename="llama-b8054-bin-macos-x64.tar.gz"
        )
        ...
        assert (
            checksum
            == "d78ccc86d8d33afd7b365f9f3310b59621c09e4d4e6dcef4cdd6482c2af1100c"
        )
    ...
    def test_get_checksum_for_release_prioritizes_bundled(self, binary_manager):
        ...
        with patch("httpx.get") as mock_get:
            ...
            mock_response.text = (
                "upstream_different_checksum llama-b8054-bin-macos-arm64.tar.gz"
            )
    ...
    def test_download_and_verify_with_bundled_checksum_mocked(
        self, binary_manager, tmp_path
    ):
    ...
    def test_missing_checksum_logs_warning(self, binary_manager, tmp_path):
        ...
        with (
            ...
            patch.object(
                binary_manager, "_get_checksum_for_release", return_value=None
            ),
        ):
...
class TestBundledChecksumIntegration:
    ...
    def test_fallback_to_upstream_if_bundled_missing(self, binary_manager):
        ...
        with patch("httpx.get") as mock_get:
            ...
            mock_response.status_code = 200
            mock_response.text = (
                "upstream_checksum_value  llama-b9999-bin-macos-arm64.tar.gz"
            )
            mock_get.return_value = mock_response
    ...
    def test_security_of_bundled_checksums(self, binary_manager):
        ...
        with tempfile.NamedTemporaryFile() as tf:
            ...
            result = binary_manager.verify_checksum(
                Path(tf.name), bundled  # Try to verify with bundled checksum
            )

            assert (
                result is False
            ), "Wrong content should fail bundled checksum verification"
